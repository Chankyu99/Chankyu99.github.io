...
고객의 구매 데이터를 기반으로 **RFM(Recency, Frequency, Monetary)** 테이블인 `user_data`를 생성했다. 이번 포스팅에서는 이 데이터를 활용해 **K-Means 클러스터링**을 수행하고, 실제 비즈니스에 적용 가능한 **고객 세그먼테이션(Segmentation) 전략**까지 도출해보려 한다.
